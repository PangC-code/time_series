...
class CustomDataset(Dataset):
    def __init__(
        ...
        self.proportion = proportion  # 保存proportion用于后续
        ...
        # 修复：先划分数据，再fit scaler
        self.data, self.scaler = self.__normalize_with_split(self.rawdata, proportion, seed)
    ...
    def __normalize_with_split(self, rawdata, proportion, seed):
        """
        修复数据泄露：只在训练集上fit scaler
        """
        # 计算训练集大小
        train_size = int(np.ceil(len(rawdata) * proportion))
        train_data = rawdata[:train_size]
        
        # 只在训练集上fit scaler
        scaler = StandardScaler()
        scaler.fit(train_data)
        
        # 用训练集的scaler transform全部数据
    ...
    def read_data(filepath, name=''):
        ...
        data = df.values
        # 修复：不再在这里fit scaler，在__normalize_with_split中处理
        return data
