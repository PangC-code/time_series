...
class Trainer(object):
    def __init__(self, config, args, model, dataloader, logger=None):
        super().__init__()
        self.config = config  # <--- 添加这一行
        self.model = model
        # self.device = self.model.betas.device   修改==========
    ...
    def sample(self, num, size_every, shape=None):
        ...
        for _ in range(num_cycle):
            ...
            # history_len = self.ema.ema_model.seq_length # 从模型获取历史长度
            # x_history = x[:, :history_len, :]
            # # history_len = self.ema.ema_model.seq_length # 从模型获取历史长度
            # # x_history = x[:, :history_len, :]

            # # 从配置中读取采样步数，如果没配，给个默认值
            # sampling_steps = config.get('forecasting', {}).get('sampling_steps', 100)

            # # 调用新的 generate_mts 方法
            # sample = self.ema.ema_model.generate_mts(x_history, steps=sampling_steps)
    ...
    def sample_forecast(self, raw_dataloader, shape=None):
        ...
        for idx, batch in enumerate(raw_dataloader):
            ...
            # 从配置中读取采样步数，如果没配，给个默认值
            # sampling_steps = config.get('forecasting', {}).get('sampling_steps', 100)
            sampling_steps = self.config.get('forecasting', {}).get('sampling_steps', 100)
            ...
            # history_len = self.ema.ema_model.seq_length
            # 真实值是从 history_len 开始的部分
            # real_future = x[:, history_len:, :]
            # reals = np.row_stack([reals, real_future.detach().cpu().numpy()])
